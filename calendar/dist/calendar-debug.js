define("tanshenghu/calendar/1.0.0/calendar-debug", [], {
    handler: {
        getId: function(id) {
            return document.getElementById(id);
        },
        getele: function(tagname) {
    ...
    createEle: function() {
        ...
        // 开始添加 title 头部
        var titleBox = _this.handler.createEle("div"), prevBtn = _this.handler.createEle("label"), curDate = _this.handler.createEle("span"), nextBtn = _this.handler.createEle("label");
        ...
        // title 部分添加完成
        // 开始创建 表格
        var table = _this.handler.createEle("table"), thead = _this.handler.createEle("thead"), week = [ "日", "一", "二", "三", "四", "五", "六" ], th = adds("th", 7, function(i) {
            if (i == 0 || i == 6) {
                this.className = "rest";
            }
    ...
    createDate: function(date) {
        ...
        date.setDate(1);
        var _this = this, adds = _this.createEle.adds, cache = _this.cache, curYear = date.getFullYear(), curMonth = date.getMonth() + 1;
        ...
        var coor = function(setData) {
            ...
            // 在title中显示 年份
            cache.curDate.innerHTML = curYear + "年" + curMonth + "月";
            // 写于数据缓存
            cache.curDate.curyear = curYear;
            cache.curDate.curmonth = curMonth;
            // 选择日历之后1号，对应的星期
            var startWeek = setData.getDay(), // 选择日历之后，当月天数，便于计算多少个tr行
            dayCount = _this.handler.getDayCount(curYear, curMonth), // 上一个月的天数
            prevMonthDayCount = _this.handler.getDayCount(curYear, curMonth - 1), // 1号之后的日期推算
            linkDay = false, // 下个月与当前月连接处
            nextMonthStartDay = 0, // 求tr的个数，不能直接用30或者31天去模，因为有时候1号从星期三开始应该把前端的三天加起来再取模
