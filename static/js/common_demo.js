...
var tsh_js_key = $.tsh.remark.key,
	tsh_js_demo = {
		...
		"common" : "<label>\/\/ 此功能开发于2014-12-21 gridsort===>列表排序功能，这个是一个json对象下面添加了很多小函数来一起完成排序的功能 <br> $('thead th').on('click', function(){<br>// 第三个可选参数的值：desc asc<br>var idx = $( this ).index();<br>window.fnTsh.gridsort.sort( 'tbody', idx, 'asc' );<br>}), <br> ObjectToArray===>将伪数组转数组.fnTsh.ObjectToArray({'0':'a','length':1}); <br> fnTsh.milliFormat( money ); // 金额千分位正则替换 <br> 2015-01-14编写验证控件，该用方法window.fnTsh.validate( 'myform' );// 验证规则、提示语都配置在页面标签中。还可以自己添加自定义规则：window.fnTsh.validate.rules.newrules = function( ele ){return /[0-9].test($(ele).val())/}，以后再优化代码，尽量做得更好吧。</label>",
